 ...
 * USE HW_DBPROG; DROP TABLE DVD;
 * Or do it from within your program!
 *
 * If you use option 1, you will keep inserting tuples in your table: cleaning it with DELETE
 * FROM DVD; can help. You can do it from within the program!
 */
import java.sql.*;
...
public class AdvancedProg {
  public static void main(String[] args) {
    ...
    Connection conn =
      ...
      // DatabaseMetaData is a class used to get information about the database: the driver, the
      // user, the versions, etc.
      ...
      while (rs.next()) {
        System.out.println(
            rs.getString(3)); // In the ResultSet returned by getTables, 3 is the TABLE_NAME.
      }
      ...
      /*
       * Method 1: Using executeUpdate, if the option allowMultiQueries=true was passed in the url given to getConnection
       * and your DBMS supports it.
       */
      ...
      // Example 1
      sqlStatement =
          "SELECT title FROM DVD WHERE Price <= ?"; // We have a string with an empty slot,
      // represented by "?".
      PreparedStatement ps =
          conn.prepareStatement(
              sqlStatement); // We create a PreparedStatement object, using that string with an
      // empty slot.
      /*
      ...
      double maxprice = 0.5;
      ps.setDouble(
          1, maxprice); // This statement says "Fill the first slot with the value of maxprice".
      ResultSet result =
          ps.executeQuery(); // And then we can execute the query, and display the results:
      ...
      rowsAffected =
          preparedStatement
              .executeUpdate(); // You can check "by hand" that this statement was correctly
      // executed.
      ...
      // To read backward and write in ResultSets, you need to have a statement with certain
      // options:
